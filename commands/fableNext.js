...
module.exports = {
  ...
  async execute(interaction) {
    ...
    console.log(highestCountPosition);
...
async function countReactions(
  ...
  if (reaction1) countReactions[0] = await getUserReactionCount(id, reaction1);

  if (reaction2) countReactions[1] = await getUserReactionCount(id, reaction2);

  if (reaction3) countReactions[2] = await getUserReactionCount(id, reaction3);

  if (reaction4) countReactions[3] = await getUserReactionCount(id, reaction4);

  if (reaction5) countReactions[4] = await getUserReactionCount(id, reaction5);

  countReactions = countReactions.filter(function (element) {
    return element !== undefined;
  });
  return (highestCountPosition = countReactions.indexOf(
